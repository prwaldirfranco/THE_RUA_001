...
# Interface
# ============================
st.set_page_config(page_title="Relatórios - POS-80", layout="wide")
st.title("📊 Relatórios Diários de Vendas")
...
with col1:
    data_inicio = st.date_input("De:", value=df["Data"].min().date() if not df.empty else datetime.now().date())
with col2:
    data_fim = st.date_input("Até:", value=df["Data"].max().date() if not df.empty else datetime.now().date())
with col3:
...
with col2:
    vendas_por_pagamento = df_filtrado.groupby("Pagamento")["Total"].sum()
